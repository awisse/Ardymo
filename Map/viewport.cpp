...
void ViewPort::move_to(const point v) {
  float to_x = view.p.x + v.x; 
  float to_y = view.p.y + v.y;
...
ScreenPt ViewPort::Map2Screen(const point p) {
  ScreenPt s_p = ScreenPt(scale * (p.x - map.p.x), scale * (p.y - map.p.y));
  return s_p;
}

bool GetLine(const line_t* line, LinePoints* s_line) {
  ...
  s_line->l0 = intersect_point(0).p.as_point();

  if (n == 2) { // Line completely cuts through the viewport
    s_line->l1 = intersect_point(1).p.as_point();
    return true;
  }
