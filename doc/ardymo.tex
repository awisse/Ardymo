...
\begin{table}
    ...
    \bottomrule
    \end{tabular}
    \caption{Distance from origin $(\xsig, \ysig)$ of sensor ray to
...
After the translation and rotation, the sensor ray has the possible
representations $(\ahat, \bhat, \Xhat, 0, 0)$ illustrated in
\tabref{tab-sensor-ray-after-transformation}
(see also Section~\ref{sec-representation-line}), depending on $\alpha$ and 
the initial form. 
\begin{table}
    \centering
    \begin{tabular}{LLL}
        \toprule
        \multicolumn{1}{l}{\sffamily Sensor Ray} &
        \multicolumn{1}{l}{\sffamily Rotation} &
        \multicolumn{1}{l}{\sffamily Transformation $(\ahat,\bhat,\Xhat)$} \\
        \midrule
        \asig=0,\,y = \bsig,\,\Xsig=0 
            & \alpha=90^\circ & \ahat=0,\,\bhat=0,\,\Xhat = \bsig - y_{r0} \\
            & \alpha=270^\circ & \ahat=0,\,\bhat=0,\,\Xhat = y_{r0} - \bsig \\
            & \alpha\notin\{90^\circ,270^\circ\} 
            & \ahat=-\frac{\sin(\alpha)}{\cos(\alpha)},\,  
                \bhat=\frac{\bsig-y_{r0}}{\cos(\alpha)},\,\Xhat=0 \\
        \asig=0,\,\bsig=0,\,x = \Xsig
            & \alpha=0^\circ & \ahat=0,\, \bhat=0,\,\Xhat=x_{r0}-\Xsig \\
            & \alpha=180^\circ & \ahat=0,\, \bhat=0,\,\Xhat=\Xsig-x_{r0} \\
            & \alpha\notin\{0^\circ,180^\circ\} 
            & \ahat=\frac{\cos(\alpha)}{\sin(\alpha)},\,
                \bhat=\frac{x_{r0}-\Xsig}{\sin(\alpha)},\,\Xhat=0 \\
        \asig\neq 0,\,y=\asig x + \bsig 
            & \cos(\alpha)+\asig\sin(\alpha)=0 
                & \ahat=0,\,\bhat=0,\,\Xhat= (\bsig-y_{r0})\sin(\alpha)-x_{r0}\\
            & \cos(\alpha)+\asig\sin(\alpha)\neq 0 
                & \ahat = \frac{\asig\cos(\alpha)-\sin(\alpha)}
                    {cos(\alpha) + \asig\sin(\alpha)},\,
                    \bhat = \frac{\asig x_{r0} + \bsig - y_{r0}}
                    {cos(\alpha) + \asig\sin(\alpha)},\,\Xhat=0\\
        \bottomrule
    \end{tabular}
    \caption{Sensor ray after
    transformation}\label{tab-sensor-ray-after-transformation}
\end{table}

The conditions for an intersection of the sensor ray with the rectangle are
summarized in \tabref{tab-intersection-rect}. We check wheter the transformed 
sensor ray has an intersection with the rectangle $(0, 0, w, h, 0)$ at the 
origin. The proofs are detailed in 
...
\begin{table}
    ...
    \begin{tabular}{LLLL}
        ...
        \multicolumn{1}{l}{\sffamily Sensor Ray} &
        \multicolumn{1}{l}{\sffamily Rotation} &
        \multicolumn{1}{l}{\sffamily Segment} &
        \multicolumn{1}{l}{\sffamily Intersection Conditions} \\
        \midrule
        \asig=0,\,y = \bsig,\,\Xsig=0 
            & \alpha=0^\circ &
                & 0 \leq x-x_{r0} \leq w\,\land\, 
                     |\bsig - y_{r0}| \leq \frac{h}{2} \\
            & \alpha=90^\circ &  
                & 0 \leq \bsig - y_{r0} \leq w \,\land\,
                    |x-x_{r0}| \leq \frac{h}{2} \\
            & \alpha=180^\circ &
                & 0 \leq x_{r0} - x\leq w\,\land\, 
                     |\bsig - y_{r0}| \leq \frac{h}{2} \\
            & \alpha=270^\circ &
                & 0\leq y_{r0}- \bsig \leq w \,\land\, 
                    |x-x_{r0}| \leq \frac{h}{2} \\
            &\alpha\notin\{0^\circ,90^\circ,180^\circ,270^\circ\} 
            & \xhat\in\{0,w\}
                & \left\{
                    \begin{tabular}{L}
                        x=x_{r0} +\frac{\xhat+(y_{r0}-\bsig)\sin(\alpha)}
                            {\cos(\alpha)}\\
                        \left|\frac{-\sin(\alpha)\xhat + \bsig - y_{r0}}
                            {\cos(\alpha)}\right| \leq \frac{h}{2}
                    \end{tabular}
                  \right. \\
            & & \yhat\in\left\{-\frac{h}{2},\frac{h}{2}\right\} 
                & \left\{
                    \begin{tabular}{L}
                        x=x_{r0}+\frac{(\bsig-y_{r0})\cos(\alpha) - \yhat}
                            {\sin(\alpha)} \\
                        0\leq \frac{\bsig - y_{r0} + \cos(\alpha)\yhat}
                            {\sin(\alpha)} \leq w
                    \end{tabular}
                \right. \\
        \asig=0,\,\bsig=0,\,x = \Xsig
            & \alpha=0^\circ &
                & 0 \leq \Xsig-x_{r0} \leq w\,
                    \land\, |y- y_{r0}| \leq \frac{h}{2} \\
            & \alpha=90^\circ & 
                & 0\leq y-y_{r0}\leq w\,
                    \land\,|\Xsig - x_{r0}|\leq\frac{h}{2}\\
            & \alpha=180^\circ &
                & 0 \leq x_{r0}-\Xsig \leq w\,\land\, 
                     |y- y_{r0}| \leq \frac{h}{2} \\
            & \alpha=270^\circ & 
                & 0\leq y_{r0}-y\leq w\,
                    \land\,|\Xsig - x_{r0}|\leq\frac{h}{2}\\
            &\alpha\notin\{0^\circ,90^\circ,180^\circ,270^\circ\} 
                ...
                & \left\{
                    \begin{tabular}{L}
                        y=y_{r0}+\frac{\xhat-(\Xsig-x_{r0})\cos(\alpha)}
                            {\sin(\alpha)} \\
                        \left|\frac{\cos(\alpha)\xhat - \Xsig + x_{r0}}
                            {\sin(\alpha)}\right| \leq \frac{h}{2} 
                    \end{tabular}
                  \right. \\
            & & \yhat\in\left\{-\frac{h}{2},\frac{h}{2}\right\} 
                & \left\{
                    \begin{tabular}{L}
                        y=y_{r0}+\frac{\yhat+(\Xsig-x_{r0})\sin(\alpha)}
                            {\cos(\alpha)}\\
                        0\leq \frac{\sin(\alpha)\yhat - \Xsig + x_{r0}}
                            {\cos(\alpha)} \leq w
                    \end{tabular}
                  \right. \\
        \asig\neq 0,\,y=\asig x + \bsig 
            & \cos(\alpha)+\asig\sin(\alpha)=0 & 
                & \left\{
                    \begin{tabular}{L}
                    |\sin(\alpha)((1+\asig^2)x+\asig(\bsig-y_{r0})-x_{r0})|
                        \leq \frac{h}{2}\\
                    0\leq (\bsig - y_{r0})\sin(\alpha)-x_{r0}\cos(\alpha)
                        \leq w\\
                    \end{tabular}
                \right. \\
            & \cos(\alpha)+\asig\sin(\alpha)\neq 0 
